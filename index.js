...
d3.json("wikipedia.json", function(error, graph) {
  ...
  var node = g.selectAll(".node")
    .data(graph.nodes)
    .enter().append("g")
    .attr("class", function(){ num_vertices++; return "node"})
    .call(force.drag)
  ...
  if (text_center){
    ...
    text.attr("dx", function(d) {return 10})
  ...
  function keydown() {
      ...
      && !d3.event.metaKey)
    {
      ...
      node.style("display", function(d) {
        return (key0||hasConnections(d))&&vis_by_type("circle")&&vis_by_node_score(d.score)?"inline":"none";
      });
